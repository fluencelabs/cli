...
const handleDocsInConfigs = async (userConfig: UserConfig) => {
  const docsInConfigsOption = isInteractive
    ? await list({
        message:
  ...
  userConfig.docsInConfigs = docsInConfigsOption === DOCS_IN_CONFIGS;
  await userConfig.$commit();

  commandObj.logToStderr(
    `If you change your mind later, modify "docsInConfigs" property in ${userConfig.$getPath()}`,
  );
};
...
const ensureUserConfig = async (): Promise<void> => {
  ...
  if (maybeUserConfig !== null) {
    const userConfig = maybeUserConfig;

    if (userConfig.docsInConfigs === undefined) {
      await handleDocsInConfigs(userConfig);
    }
  ...
  commandObj.logToStderr(
    `If you change your mind later, modify "countlyConsent" property in ${newUserConfig.$getPath()}`,
  );

  await handleDocsInConfigs(newUserConfig);
...
> {
  ...
  await handleCliVersion(maybeFluenceConfig?.cliVersion);
