...
export async function generateUserProviderConfig({
  ...
  do {
    ({ computePeersCounter, isAddingMoreComputePeers } = await addComputePeer({
      computePeersCounter,
      numberOfNoxes,
      userProvidedConfig,
  ...
  do {
    ({ offersCounter, isAddingMoreOffers } = await addOffer({
      offersCounter,
      userProvidedConfig,
      isAddingMoreOffers,
    }));
...
type AddComputePeerArgs = {
  computePeersCounter: number;
  numberOfNoxes: number | undefined;
  userProvidedConfig: UserProvidedConfig;
  isAddingMoreComputePeers: boolean;
};

async function addComputePeer({
  computePeersCounter,
  numberOfNoxes,
  userProvidedConfig,
  isAddingMoreComputePeers,
  ...
  return { computePeersCounter, isAddingMoreComputePeers };
}

type AddOfferArgs = {
  offersCounter: number;
  userProvidedConfig: UserProvidedConfig;
  isAddingMoreOffers: boolean;
};
...
async function addOffer({
  ...
  return { offersCounter, isAddingMoreOffers };
}
