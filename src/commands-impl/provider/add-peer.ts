...
import type AddPeer from "../../commands/provider/add-peer.js";
...
export async function addPeerImpl(
  this: AddPeer,
  command: typeof AddPeer,
): Promise<void> {
  ...
  for (const peerId of peerIds) {
    ...
    commandObj.logToStderr(
      `Added ${color.yellow(
        workersCount,
      )} worker slots. Compute peer ${color.yellow(peerIds)} has ${color.yellow(
        free,
