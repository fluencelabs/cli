...
You are welcome to contribute to flox!
...
-   To use flox in the development mode, run: `./bin/dev.js` (types are not checked in development mode because it's faster and more convenient to work with. Use typechecking provided by your IDE during development)
-   To use flox in the production mode, run `pnpm run build` first, then run: `./bin/run.js`. If you want to make sure you are running the actual package the users will use, do `pnpm build`, `pnpm pack` and install this tar package (the approach is used for tests)
...
-   Don't use colors inside commands descriptions. They can't be rendered to markdown and they will not be rendered to users of the packaged flox anyway, when they run `--help`
