...
>(
  ...
  return async (): Promise<InitializedReadonlyConfig<LatestConfig> | null> => {
    ...
    try {
      ...
      const documentationLinkComment = `# Documentation for CLI v${commandObj.config.version}: https://github.com/fluencelabs/cli/tree/fluence-cli-v${commandObj.config.version}/packages/cli/package/docs/configs/${name.replace(
        `.${YAML_EXT}`,
        "",
      )}.md`;
