...
- To run cli in development mode use: `npm run dev` (e.g. `npm run dev -- init`)
- To run cli in production mode run `npm run build` first, then use: `./bin/run.js`. If you wanna make sure you are running the actual package the users will use - do `npm run build`, `npm pack` and install this tar package (this approach is used for tests)
...
```
USAGE
  $ fluence aqua [-i <value>] [-o <value>] [--import <value>] [--common-js | [--air | --js | ]]
